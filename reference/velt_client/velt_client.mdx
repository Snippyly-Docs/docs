...
`apiKey: string` -  Your API key to make authenticated calls to Snippyly Servers and track SDK usage.
...
A `location` is always under a `document` in hierarchy. Once you set this, all Snippyly features will now be attributed to this specific `location` under the `document`. In a way all Snippyly data will be stored in a hierarcy like this: `document`/`location`. 
...
Removes the specific location. Now all Snippyly features will attributed at `documentId` level.
...
To set the dark mode of Snippyly components.
